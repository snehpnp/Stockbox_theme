  ...
  return (
    <>
      <form onSubmit={formik.handleSubmit}>
        ...
        <div className="row">
          {fieldtype.map((field, index) => {
            if (field.type === "select") { // Select input ka case
              return (
                <div className={`col-md-${field.col_size}`} key={index}>
                  <div className="mb-3">
                    <div className="form-group">
                      <label className="form-label">{field.label}</label>
                      <select
            ...
            } else if (field.type === "checkbox") { // Checkbox input ka case
              return (
                <div className={`col-md-${field.col_size}`} key={index}>
                  <div className="mb-3">
                    <label className="form-label">{field.label}</label>
            ...
            } else if (field.type === "textarea") { // Yahan textarea ko handle kiya hai
              return (
                <div className={`col-md-${field.col_size}`} key={index}>
                  <div className="mb-3">
                    <label className={`col-lg-${field.label_size}`}>{field.label}</label>
                    <textarea
                      ...
                      onBlur={formik.handleBlur}
                      value={formik.values[field.name]}
                      rows={5} // Aap rows ko adjust kar sakte ho
                    />
                    {formik.touched[field.name] && formik.errors[field.name] ? (
                      <div className="text-danger">{formik.errors[field.name]}</div>
                    ) : null}
