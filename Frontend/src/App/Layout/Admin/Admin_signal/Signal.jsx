    ...
    try {
      ...
      if (response.status) {
        if (response.data?.length > 0) {
          ...
          const csvArr = filterdata.map((item) => {
            const entryType = `${item?.calltype} ${item?.stock} ${
              item?.expirydate ? `Expiry: ${item.expirydate}` : ""
            } ${
              item?.optiontype ? `Option: ${item.optiontype}` : ""
            } Entry Type: ${item?.entrytype} Price: ${item?.price} Target: ${
              item?.tag1
            } ${item?.tag2 ? `/${item.tag2}` : ""} ${
              item?.tag3 ? `/${item.tag3}` : ""
            ...
            return {
              OpenSignal: `${fDateTimeH(item?.created_at)}  Segment: ${
                item.segment === "C"
                  ? "CASH"
                  : item.segment === "O"
                  ? "OPTION"
          ...
          <form>
            <div className="row">
              ...
              <div className="col-md-12">
                <div className="card-body ">
                  <ul
                    className="list-unstyled signal-message-list"
